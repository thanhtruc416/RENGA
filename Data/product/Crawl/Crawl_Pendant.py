"""
PNJ Mặt Dây Chuyền Crawler — RENGA Schema Edition
====================================================
Crawl tối đa 500 mặt dây chuyền từ PNJ trong khoảng giá 200.000đ - 15.000.000đ
Map vào 4 bảng RENGA: CATEGORY, PRODUCT, PRODUCT_VARIANT, PRODUCT_IMAGE

URL nguồn : https://www.pnj.com.vn/mat-day-chuyen/
Output    : Data/Raw/pnj_mat_day_chuyen.json
...
GHI CHÚ KỸ THUẬT:
  • STONE / MATERIAL KHÔNG được map ở đây — đó là bảng của The Studio (customization)
  • Thông tin chất liệu, đá crawl được → lưu vào PRODUCT.description dạng text
  • category_id, created_by_admin_id → điền khi INSERT vào DB (không có từ crawl)
─────────────────────────────────────────────────────────────────────────────
"""
...
# ── Cấu hình ──────────────────────────────────────────────────────────────
BASE_URL     = "https://www.pnj.com.vn/mat-day-chuyen/"
MIN_PRICE    = 200_000
...
# Danh mục cha: Dây chuyền | Danh mục con: Mặt dây chuyền
CATEGORY_DATA = {
    "parent": {
    ...
    "child": {
        "category_name": "Mặt dây chuyền",
        "slug":          "mat-day-chuyen",
        "display_order": 1,
        "is_active":     True,
        # parent_id → trỏ tới category_id của "Dây chuyền", điền khi INSERT
    }
}
...
def build_description(product_name: str, features: list, raw_description: str) -> str:
    ...
    size   = get_feature(features, "kích thước mặt", "kích thước", "chiều dài mặt")
    ...
    if size:
        parts.append(f"Kích thước mặt: {size}")
    if weight:
...
def parse_product(item: dict, detail: dict | None) -> dict:
    ...
    if detail:
        ...
        # PRODUCT.description = gộp thông tin chất liệu/đá vào text
    ...
    return {
        ...
        "product": {
            ...
            "status":       map_product_status(list_price),  # ENUM ACTIVE/INACTIVE/OUT_OF_STOCK
        },
...
async def main():
    print("=" * 60)
    print("PNJ Mặt Dây Chuyền Crawler — RENGA Schema Edition")
    ...
    async with async_playwright() as p:
        ...
        while True:
            ...
            print(f"lấy {len(in_range)}/{len(items)} sp trong khoảng giá (tổng: {len(raw_items)})")
    ...
    out_path   = os.path.join(raw_dir, "pnj_mat_day_chuyen.json")
    ...
    output = {
        ...
        # category_id điền tay hoặc generate khi INSERT
    ...
    print(f"  Có nhiều variants (size): {multi_var}/{len(all_products)}")
    print(f"  Tổng ảnh: {total_images} ({total_images/len(all_products):.1f} ảnh/sp)")
