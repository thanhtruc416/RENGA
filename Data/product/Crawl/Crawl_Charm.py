"""
PNJ Charm Crawler — RENGA Schema Edition
=========================================
Crawl tối đa 500 charm từ PNJ trong khoảng giá 200.000đ - 10.000.000đ
Map vào 4 bảng RENGA: CATEGORY, PRODUCT, PRODUCT_VARIANT, PRODUCT_IMAGE

URL nguồn : https://www.pnj.com.vn/charm/
Output    : Data/Raw/pnj_charm.json
...
GHI CHÚ KỸ THUẬT:
  • Trang /charm/ dùng CS-Cart (HTML thuần) – scrape listing HTML trực tiếp.
  • Giá thực lấy từ <input id="cdp_product_price{id}" value="...">
  • Trang listing: /charm/, /charm/page-2/, ...
...
# ── Cấu hình ──────────────────────────────────────────────────────────────
BASE_URL     = "https://www.pnj.com.vn/charm/"
MIN_PRICE    = 200_000
...
# Charm thuộc nhóm "Bộ sưu tập đặc biệt & Khác" trong đồ án
# Đây là danh mục độc lập (không có cha rõ ràng như nhẫn/dây chuyền)
CATEGORY_DATA = {
    "parent": {
        "category_name": "Charm",
        "slug":          "charm",
        "display_order": 5,          # nhóm 5 trong danh mục sản phẩm đồ án
        "is_active":     True,
        "parent_id":     None,       # đây là danh mục gốc, không có cha
    },
    # Không có "child" — charm là danh mục duy nhất, không cần tách con
}
...
def build_description(product_name: str, features: list, raw_description: str) -> str:
    """
    Gộp thông tin chất liệu, đá, kích thước charm vào PRODUCT.description dạng text thuần.
    ...
    size       = get_feature(features, "kích thước", "chiều dài", "chiều rộng",
                             "size", "dimensions")
...
async def scrape_listing_page(context, page_num: int) -> tuple[list, bool]:
    ...
    try:
        ...
        # Normalize thumbnail → full-size ở Python (tránh gọi Python function trong JS)
        for item in items:
    ...
    try:
        ...
        fallback = await page.evaluate("""
            () => {
                ...
                return { images, features, full_description: desc };
            }
        """)
...
def parse_product(item: dict, detail: dict | None) -> dict:
    ...
    # Default 1 variant — charm không có size như nhẫn
    variants = [{
    ...
    if detail:
        ...
        # PRODUCT.description = gộp thông tin chất liệu/đá/kích thước vào text
        ...
        # Variants theo màu/kiểu dáng (nếu có)
        ...
        if size_prices:
            ...
            for sz, price_info in size_prices.items():
                ...
                style_variants.append({
                    "variant_name":   sz,
                    "size_value":     None,   # charm không có size số
    ...
    return {
        ...
        "product": {
            ...
            "description":  description,         # chất liệu/đá/kích thước gộp vào đây
        ...
        # size_value = None vì charm không có size số
        "variants": variants,
...
async def main():
    print("=" * 60)
    print("PNJ Charm Crawler — RENGA Schema Edition")
    ...
    out_path   = os.path.join(raw_dir, "pnj_charm.json")
    ...
    output = {
        ...
        "_insert_notes": {
            ...
            "size_value_note": "size_value = None vì charm không có size số; variants phân biệt theo màu/kiểu dáng nếu có",
        },
