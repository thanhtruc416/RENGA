...
export async function reconcileTier(conn: PoolConnection, clientId: string): Promise<void> {
...
export async function awardPurchasePoints(
  ...
  await reconcileTier(conn, clientId);
}
